 ...
 * @link    http://phpsx.org
 */
class Swagger implements ParserInterface, ParserCollectionInterface
{
    /**
     * @var string|null
     */
    ...
    public function parse($schema, $path)
    {
        ...
        if (isset($paths[$path])) {
            ...
            throw new RuntimeException('Could not find resource definition "' . $path . '" in Swagger schema');
        }
    }
    ...
    {
        list($properties, $required) = $this->parseParameters('body', $data);
    ...
    public static function fromFile($file, $path)
    {
        if (!empty($file) && is_file($file)) {
            ...
            $parser   = new Swagger($basePath);

            return $parser->parse(file_get_contents($file), $path);
        } else {
            throw new RuntimeException('Could not load Swagger schema ' . $file);
        }
    }
}
